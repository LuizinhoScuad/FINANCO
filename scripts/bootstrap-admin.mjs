 ...
 *
 * Uso:
 *   node scripts/bootstrap-admin.mjs --list              lista as contas (só lê)
 *   node scripts/bootstrap-admin.mjs <email>             mostra o que faria
 *   node scripts/bootstrap-admin.mjs <email> --confirmar aplica de verdade
 *
...
async function promover(email) {
  ...
  console.log(`  claims depois.: role=ADMIN status=ACTIVE`);
  ...
  await auth.setCustomUserClaims(usuario.uid, { role: "ADMIN", status: "ACTIVE" });

  await perfilRef.set(
    {
      name: usuario.displayName ?? usuario.email?.split("@")[0] ?? "Administrador",
