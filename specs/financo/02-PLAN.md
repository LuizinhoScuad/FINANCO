...
app/
  (app)/
    dashboard|transacoes|contas|categorias|orcamentos/    pessoal — mantido
    despesas/                                             colaborador
    admin/usuarios|aprovacoes|relatorios/                 gestor
...
actions/                     server actions FINAS — só orquestram
  ...
  expenses · admin-users

lib/
  core/                      regra de negócio e acesso a dados, tipados
    repositories/*.repo.ts
    money.ts                 aritmética em centavos
    expense-status.ts        máquina de estados (pura, testável)
    exports/                 pdf-report.ts · xlsx-report.ts
...
| D3 | `expenses` em coleção de topo com campo `userId` | A consulta dominante é a do admin vendo todas as pendentes; em subcoleção isso exigiria consulta de grupo e regra mais frágil. Dado pessoal continua sob `users/{uid}/` |
| D4 | Máquina de estados pura em `expense-status.ts` | Mesma fonte de verdade para action, regras do banco e teste — três lugares onde a regra precisa concordar |
| D5 | Lote de pagamento (`paymentBatches`), não baixa individual | Reflete como o ressarcimento acontece de fato: fecha-se um período por pessoa e paga-se de uma vez. Marcar tudo via `writeBatch` é atômico (Art. 2) |
...
| D10 | Centavos (inteiro) no módulo de ressarcimento | Evita erro de ponto flutuante onde ele custaria dinheiro de terceiro. O módulo pessoal só migra se a auditoria apontar erro real (Art. 10) |
...
0 ─ Higiene ✔ ──> 1 ─ Fundação SDD ✔
                  ...
                  4 ─ Integridade ──> 5 ─ Backup e erros
                        ...
                        v
                  6 ─ Ressarcimento ──> 7 ─ Aprovação, lotes e relatórios
                        │
                        v
                  9 ─ Testes, CI, auditoria e go-live
```

**Segurança e integridade antes de funcionalidade.** Nenhum colaborador é
convidado antes da Fase 3; nenhum dinheiro de terceiro é registrado antes da
Fase 4 estar concluída. O Guardião vem depois que existe o que vigiar.
...
| [6](fases/FASE-6-RESSARCIMENTO.md) | Modelo, regras e fluxo do colaborador | pendente |
| [7](fases/FASE-7-ADMIN-LOTES-RELATORIOS.md) | Aprovação, fechamento de lote, relatórios, PDF e XLSX | pendente |
...
Estimativa: 8 a 10 sessões restantes (a Fase 4 provavelmente ocupa duas).
...
| Brecha na regra de transição (colaborador aprovar a própria despesa) | Mesma máquina de estados valida na action e na regra do banco; matriz estado × papel × operação testada caso a caso; auditoria na Fase 9 |
